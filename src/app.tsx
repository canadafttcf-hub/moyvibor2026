...
const PARTIES = [
  { code: "ER",   name: "Единая Россия", color: "#1e40af", emoji: "🐻", leader: "Дмитрий Медведев", descr: "Консервативная партия власти. Приоритеты: стабильность, порядок, суверенитет.", logo: "er_logo.png" },
  { code: "KPRF", name: "КПРФ", color: "#dc2626", emoji: "☭", leader: "Геннадий Зюганов", descr: "Левая оппозиция. За национализацию, social'ное равенство и советские стандарты.", logo: "kprf.png" },
  { code: "LDPR", name: "ЛДПР", color: "#ea580c", emoji: "🦅", leader: "Леонид Слуцкий", descr: "Национал-популизм. Жёсткая внешняя политика, поддержка армии и ограничение миграции.", logo: "ldpr.png" },
  { code: "NL",   name: "Новые люди", color: "#0d9488", emoji: "🌿", leader: "Алексей Нечаев", descr: "Либеральный центр. Поддержка бизнеса, обновление элит и цифровизация.", logo: "nl_logo.png" },
  { code: "SR",   name: "Справедливая Россия", color: "#7c3aed", emoji: "⚖️", leader: "Сергей Миронов", descr: "Социал-демократия. Справедливое распределение благ и патриотизм.", logo: "sr_logo.png" },
  { code: "YAB",  name: "Яблоко", color: "#16a34a", emoji: "🍏", leader: "Николай Рыбаков", descr: "Либеральная оппозиция. Права человека, европейский путь и свобода слова.", logo: "yabl.png" },
  { code: "RPPS", name: "Партия Пенсионеров", color: "#9a3412", emoji: "🧓", leader: "Владимир Ворожцов", descr: "Социальный патернализм. Защита интересов старшего поколения и качественная медицина.", logo: "pp_logo.png" },
  { code: "KR",   name: "Коммунисты России", color: "#991b1b", emoji: "🚩", leader: "Сергей Малинкович", descr: "Радикальные левые. Возврат к плановой экономике и идеям Ленина.", logo: "communist_logo.ppg" },
  { code: "ZEL",  name: "РЭП Зелёные", color: "#22c55e", emoji: "🌱", leader: "Андрей Нагибин", descr: "Экология. Зелёные технологии и жесткая защита окружающей среды.", logo: "green.png" },
  { code: "ROD",  name: "Партия Родина", color: "#475569", emoji: "⚔️", leader: "Алексей Журавлёв", descr: "Национал-консерватизм. Военная мощь и жёсткий государственный суверенитет.", logo: "rodina.ppg" },
  { code: "PPD",  name: "Партия прямой демократии", color: "#0284c7", emoji: "📱", leader: "Олег Артамонов", descr: "Технократия. Цифровое управление, электронное голосование и блокчейн-демократия.", logo: "ppd.png" },
  { code: "GP",   name: "Гражданская Платформа", color: "#d97706", emoji: "💼", leader: "Рифат Шайхутдинов", descr: "Бизнес-консерватизм. Экономическая свобода предпринимательства при лояльности государству.", logo: "platforma.png" }
];
...
const SCALE_LABELS: Record<number, string> = { 1: "Против", 2: "Скорее нет", 3: "Нейтрально", 4: "Скорее да", 5: "Согласен" };
...
function calcScores(answers: Record<number, number>, weights: Record<string, number>) {
  ...
  Object.entries(answers).forEach(([qn, val]) => {
    ...
    const w = (sec && weights[sec] !== undefined) ? weights[sec] : 1;
    const row = MATRIX[q];
    if (!row) return;
    pKeys.forEach((p, i) => {
...
function calcAxes(answers: Record<number, number>) {
  return AXES.map(ax => {
...
function PartyLogo({ party, size = "small" }: PartyLogoProps) {
  ...
  // Gracefully fallback to standard Emoji badge if there's no logo defined or the image fails to load
  ...
  return (
    <div className="app">
      ...
      <div className="glass-card" style={{ borderBottom: "4px solid #6366f1" }}>
        <div className="arch-label">Ваш political'ный архетип</div>
