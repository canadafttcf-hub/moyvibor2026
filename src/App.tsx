...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Fixed emoji badges to make them display as elegant solid circles when no images are present */
  .party-emoji-badge {
    width: 38px;
    height: 38px;
...
function PartyLogo({ party, size = "small" }: PartyLogoProps) {
  ...
  // If there's no logo or loading failed, we fall back to the newly styled solid emoji circle
  if (!party.logo || imgError) {
    return (
      <div className={emojiClass} style={{ color: party.color, borderColor: party.color }}>
        {party.emoji}
      </div>
    );
  }
  ...
  return (
    <div className={badgeClass} style={{ borderColor: party.color }}>
      <img 
        src={party.logo} 
        alt={party.name} 
        onError={() => {
...
export default function App() {
  ...
  const handleShare = () => {
    ...
    // Fast reliable clipboard writing
